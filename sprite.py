...
class Bloque(pygame.sprite.Sprite): #Hereda de pygame.sprite.Sprite
    def __init__(self, game, x, y, text): #refencia a la instancia de juego principal, x y posiciones en la cuadricula, texto mostrados en bloques
        ...
        #Llama a actualizacion para posicionar correctamente el rectangulo
        self.actualizacion()
...
class Boton:
    ...
    def dibujar(self, screen):
        pygame.draw.rect(screen, self.color, (self.x, self.y, self.width, self.height))# Dibujar el rectángulo del boton
        text_surface = self.font.render(self.text, True, self.text_color) #Renderizar el texto
        text_rect = text_surface.get_rect(center=(self.x + self.width // 2,self.y + self.height // 2)) #Obtener el rectangulo del texto y centrarlo en el boton
